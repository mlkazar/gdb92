...
#ifndef NOKAZAR
void
kazar_prepare_to_resume()  {
  /* hide any user threads, since gdb doens't know how to ignore them, and
   * then select a real thread to reestablish reasonable state.
   */
  struct inferior *infp;
  infp = current_inferior();
  if (infp->thread_list->ptid.tid() != 0) {
    hide_uthreads();
    thread_select("Continue", infp->thread_list);
  }
}
#endif
