...
hide_uthreads()
{
  ...
  tp = infp->thread_list;
  if (tp->ptid.tid() != 0) {
    /* move tp from head of thread_list to head of uthread_list */
    printf("Hiding user threads\n");
    infp->thread_list = tp->next;
    tp->next = infp->uthread_list;
    infp->uthread_list = tp;
  }
}
...
unhide_uthreads()
{
  ...
  infp = current_inferior();
  tp = infp->uthread_list;
  if (tp != 0) {
    /* move tp from head of thread_list to head of uthread_list */
    printf("Unhiding user threads\n");
    infp->uthread_list = tp->next;
    tp->next = infp->thread_list;
    infp->thread_list = tp;
  }
}
...
#ifndef NOKAZAR
  /* if we're switching to a normal thread (tid == 0), remove any user threads; this will
   * allow 'continue' command etc to work without trying to restore registers from
   * the user thread.
   */
  if (tp->ptid.tid() == 0) {
    hide_uthreads();
...
/* structure represents a thread queue, and a thread element.  It
 * obviously needs to stay in sync with task.h.
 */
#include "linux-tdep.h"

/* command to load user threads into system; need to clear out the old ones as well */
void
uthread_command (const char *tidstr, int from_tty)
{
  struct thread_info *tinfop;
  struct regcache *parentRegcachep;
  struct gdbarch *archp;
  const struct address_space *aspacep;
  struct inferior *infp;
  ...
  archp = parentRegcachep->arch();
  // aspacep = get_regcache_aspace(parentRegcachep);
  aspacep = parentRegcachep->aspace();

#if 0
  /* lookup the symbol's address that points to the list of user-level tasks;
   * store the address of the structure in addr.  Leftover from when we loaded
   * all threads in this command.
   */
...
#else
  tidp = tidstr;
  taskAddr = (long long) get_ulongest(&tidp, '\0');
#endif
  ...
  addedThreads = 0;
  {
    ...
    if (!tinfop) {
      ...
      memset((void *) tinfop, 0, sizeof(*tinfop));
      tinfop->ptid = ptid_t(infp->pid, infp->thread_list->ptid.lwp() , (long) 10001);
      tinfop->inf = infp;
      ...
      get_thread_arch_aspace_regcache (tinfop->ptid, archp, (struct address_space *) aspacep);
      ...
      tinfop->next = infp->thread_list;
      infp->thread_list = tinfop;
	...
	taskAddr -= offsetBack;
      ...
      /* just fill the register cache from the thread */
      read_data_memory(archp,
		       /* !stack */ 0,
		       (long) taskAddr,
		       (gdb_byte *) &thr,
